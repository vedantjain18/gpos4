...
const SalesQuotation = () => {
    ...
    const fetchAllIitems = async () => {
        try {
            const fetchedItemsResponse = await axiosInstance(`/salesmgmt/item-master-search-by-name?business_id=${business_id}&employee_master_id=${employee_master_id}`);
...
//     const employee_master_id = 1;
    ...
    useEffect(() => {
        axiosInstance.get(`/salesmgmt/api/sales-quotation-pending?business_id=${business_id}&employee_master_id=${employee_master_id}`)
            .then((res) => res.json())
